...
public class Cooking extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        total = findViewById(R.id.totalForCooking);
        cookingTotal = findViewById(R.id.cookingTotal);
    }

    @Override
    protected void onResume() {
        super.onResume();
        cookingTotal.setText(DiaryToatal+" L");
    ...
    public void DiaryTotal111(String date){
        Cursor cursor = baseHelper.makeQuiry("select MARKS FROM Categories WHERE Date ='" + date + "' and NAME ='cooking' ");
        ArrayList<EditModel> www = new ArrayList<>();
        if (cursor.getCount() == 0) {
            totalForDiary = 0;
            DiaryToatal = "0";
