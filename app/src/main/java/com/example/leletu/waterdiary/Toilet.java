...
public class Toilet extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toiletTotal = findViewById(R.id.toiletTotal);
    }

    @Override
    protected void onResume() {
        super.onResume();
        toiletTotal.setText(DiaryToatal +" L");
    ...
    public void DiaryTotal111(String date){
        Cursor cursor = baseHelper.makeQuiry("select MARKS FROM Categories WHERE Date ='" + date + "' and NAME ='toilet' ");
        ArrayList<EditModel> www = new ArrayList<>();
        if (cursor.getCount() == 0) {
            totalForDiary = 0;
            DiaryToatal = "0";
